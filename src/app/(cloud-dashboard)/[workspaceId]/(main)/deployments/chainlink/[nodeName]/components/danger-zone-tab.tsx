...
export const DangerZoneTab: React.FC<DangerZoneTabProps> = ({ node }) => {
  ...
  async function onDeleteChainlinkNode() {
    const url = qs.stringifyUrl({
      url: `/chainlink/nodes/${node.name}`,
      query: { workspace_id: params.workspaceId },
    });
    await client.delete(url);
    router.push(`/${params.workspaceId}/deployments/chainlink`);
    router.refresh();
    toast({
  ...
  return (
    <>
      ...
      <DeprecatedAlertModal
        ...
        description={`This action cann't be undone. This will permnantly delete (${node.name}) Chainlink Node.`}
      >
        <DeleteNodeForm nodeName={node.name} onDelete={onDeleteChainlinkNode} />
      </DeprecatedAlertModal>
    </>
  );
};
